...
pub fn plaintext_version_is_valid<P: consensus::Parameters>(
    ...
    if params.is_nu_active(Canopy, height) {
        ...
        // Accept both pre-Canopy (0x01) and post-Canopy (0x02) note plaintext versions.
        // On chains where Canopy is not activated (e.g. Zclassic), some wallet software
        // may create notes with version byte 0x02 (ZIP-212 format). These are valid
        // on-chain (consensus doesn't check the plaintext version byte), so we must
        // accept them to avoid silently missing notes during trial decryption.
...
/// Raw compact decryption WITHOUT CMU or ESK validity checks.
///
/// This function performs the same decryption as `try_sapling_compact_note_decryption`
/// but skips the CMU verification step. It returns the raw parsed fields
/// (version byte, diversifier, value, rcm/rseed bytes) for any output that
/// successfully key-agrees and decrypts.
///
/// Used for diagnostic purposes: to detect notes belonging to this wallet whose
/// CMU in the boost file doesn't match the computed CMU from the decrypted data.
/// If this function finds a match that `try_sapling_compact_note_decryption` misses,
/// the boost file has a wrong CMU for that output.
pub fn try_sapling_raw_compact_decryption<P: consensus::Parameters>(
    _params: &P,
    _height: BlockHeight,
    ivk: &PreparedIncomingViewingKey,
    epk_bytes: &[u8; 32],
    ...
    let epk: Option<EphemeralPublicKey> = EphemeralPublicKey::from_bytes(epk_bytes).into();
    let prepared_epk = PreparedEphemeralPublicKey::new(epk?);

    // Step 2: Key agreement
    let shared_secret = prepared_epk.agree(ivk);

    // Step 3: KDF
    let key = shared_secret.kdf_sapling(&ephemeral_key);

    // Step 4: Decrypt first 52 bytes using ChaCha20 stream cipher
    let mut plaintext = [0u8; 52];
    plaintext.copy_from_slice(compact_ciphertext);
    let mut keystream = ChaCha20::new(key.as_ref().into(), [0u8; 12][..].into());
    keystream.seek(64);
    keystream.apply_keystream(&mut plaintext);

    // Step 5: Parse WITHOUT checking CMU or ESK
    let version_byte = plaintext[0];
    if version_byte != 0x01 && version_byte != 0x02 {
        return None;
    }
    ...
    rcm_bytes.copy_from_slice(&plaintext[20..52]);

    // Step 6: Verify diversifier is valid for this ivk (lightweight ownership check)
    let div = Diversifier(diversifier);
    let _pk_d = DiversifiedTransmissionKey::derive(ivk, &div)?;

    Some((version_byte, diversifier, value, rcm_bytes))
}
