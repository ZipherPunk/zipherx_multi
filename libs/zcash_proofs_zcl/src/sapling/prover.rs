...
impl SaplingProvingContext {
    ...
    /// FIX #1326: Generate a spend proof WITHOUT accumulating into the context.
    /// Returns (proof, value_commitment, rk, rcv) where rcv is needed for later
    /// accumulation via `accumulate_spend()`.
    ///
    /// This method is thread-safe (&self, not &mut self) and can be called in
    /// parallel from multiple threads via rayon. Each call uses OsRng (thread-safe
    /// kernel CSPRNG) for independent randomness.
    ///
    /// The proving_key is &Parameters<Bls12> (immutable shared reference) — safe
    /// for concurrent reads by Rust's ownership model.
    #[allow(clippy::too_many_arguments)]
    pub fn spend_proof_detached(
        proof_generation_key: ProofGenerationKey,
        diversifier: Diversifier,
        rseed: Rseed,
        ar: jubjub::Fr,
        ...
        // NOTE: Do NOT accumulate bsk/cv_sum here — caller does this after parallel completion
        ...
        // Create proof — THE HEAVY PART (~1 second)
        let proof =
            create_random_proof(instance, proving_key, &mut rng).expect("proving should not fail");
        ...
        // Return proof + rcv (caller accumulates rcv and cv into context)
        Ok((proof, value_commitment, rk, rcv))
    }
    ...
    pub fn accumulate_spend(&mut self, cv: &ValueCommitment, rcv: &ValueCommitTrapdoor) {
        self.bsk += rcv;
        self.cv_sum += cv;
    }
