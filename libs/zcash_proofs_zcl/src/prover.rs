...
impl LocalTxProver {
    /// FIX #1326: Generate a spend proof without accumulating into context.
    /// Thread-safe (&self) — can be called from multiple rayon threads concurrently.
    /// Returns (proof_bytes, cv, rk, rcv) for later accumulation.
...
impl TxProver for LocalTxProver {
    ...
    /// FIX #1326: Detached spend proof for parallel generation via rayon.
    /// Delegates to the inherent method on LocalTxProver.
    ...
    /// FIX #1326: Accumulate detached proof results into proving context.
    fn accumulate_spend(
        &self,
        ctx: &mut Self::SaplingProvingContext,
        cv: &ValueCommitment,
        rcv: &zcash_primitives::sapling::value::ValueCommitTrapdoor,
    ) {
        ctx.accumulate_spend(cv, rcv);
    }
}
